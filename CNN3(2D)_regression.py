...
from keras.layers import Dense, Conv2D, Flatten, Dropout, BatchNormalization, Activation, MaxPooling1D
...
def cnn_model_fn(num_of_classes):
        model = Sequential()
        # Block 1
        model.add(Conv2D(64, (8,3), strides=1, activation='relu', kernel_initializer= 'normal', padding="same", input_shape=(128,t,1)))
        #model.add(Dropout(0.1))   # Don't add dropout in conv
        model.add(Conv2D(64, (8,3), strides=1, activation='relu', kernel_initializer= 'normal', padding="same"))
        # Block 2
        #model.add(Conv2D(64, (4,2), strides=1, activation='relu', kernel_initializer= 'normal', padding="same"))
        #model.add(Conv2D(128, (3,3), strides=1, activation='relu', kernel_initializer= 'normal', padding="same"))
...
def reshape(training_data_fit):
    ...
    training_data_fit = np.reshape(training_data_fit, (training_data_fit.shape[2], 128, training_data_fit.shape[0]))   
...
if __name__ == "__main__":
    ...
    training_data = np.ones((t,128))
    labels = []   # Training labels
    file_marker = []
    # Match csid + target class with file names
    for i in range(len(csid[0:100])):
        for j in range(len(csv_list)):
            if csv_list[j].startswith(str(csid[i])) and target in csv_list[j]:
                ...
                for length in range(0, temp_data.shape[0], t):
                    try:
                        segment = temp_data[length:length+t]
                        training_data = np.dstack((training_data, segment))
